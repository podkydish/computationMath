...
public class Task {
    ...
    private static void calculateNewton(Operationable func, Operationable der, double x) {
        int numberIteration;
        for (numberIteration = 1; Math.abs(func.calculate(x)) > eps; numberIteration++) { // Задаём условия для нахождения приближённого значения

            if (der.calculate(x) == 0) {
                break;
            }
            x = x - func.calculate(x) / der.calculate(x);   // Формула нахождения следующего х
